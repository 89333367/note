...
> **配合 `/handoff` 技能无缝交接**：如果你安装了 mattpocock 技能合集（见 `6.3`），在结束当前任务、准备开新会话前，可以先运行 `/handoff` 技能。它会把当前对话压缩成一份交接文档（Markdown），记录任务背景、已完成的工作和待办事项。开新会话后，在输入框中用 `@` 引用这份交接文档，AI 就能快速了解之前的全部上下文，继续未完成的工作，不会因为 `/new` 而丢失记忆。
