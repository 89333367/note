...
> **配合 `/handoff` 技能无缝交接**：如果你安装了 mattpocock 技能合集（见 `6.3.1`），在结束当前任务、准备开新会话前，可以先运行 `/handoff` 技能。它会把当前对话压缩成一份交接文档（Markdown），记录任务背景、已完成的工作和待办事项。开新会话后，把交接文档的**完整路径**发给 AI（如 `C:\Users\你的用户名\AppData\Local\Temp\handoff-xxx.md`，由 AI 用 read 工具读取），AI 就能快速了解之前的全部上下文，继续未完成的工作，不会因为 `/new` 而丢失记忆。注意：交接文档保存在系统临时目录而非项目目录，`@` 自动补全无法直接找到，且 AI 读取工作区外文件会触发权限确认。
...
- 上下文接近上限时 OpenCode 会自动压缩，把早期对话浓缩成一份 AI 生成的摘要——但摘要内容**不可控**，你强调过的关键信息（约定、决策、数字）很可能在压缩中丢失
- `/compact` 手动触发与自动压缩机制相同，只是换了触发时机，同样会丢细节
- 因此**不要依赖压缩来保留重要信息**：关键内容应写进文件（如 `AGENTS.md`、`docs/`），或直接用 `/handoff` 落盘交接

**正确的做法**：

1. 当任务快完成、上下文也占用较多时，先运行 `/handoff`（见 `8.2`）生成交接文档，把当前进展和待办事项保存下来
2. 用 `/new` 开启新会话，把交接文档的**完整路径**发给 AI（如 `C:\Users\你的用户名\AppData\Local\Temp\handoff-xxx.md`），让 AI 读取后继续工作
3. 这样既能保住工作成果，又能让 AI 在新会话中保持清醒的状态

> 提示：**`/handoff` 必须在自动压缩发生之前执行**。handoff 是基于当前完整对话历史生成文档的，一旦上下文已满、被自动压缩过，高信号信息已经被摘要抹掉了，此时再 handoff 也只能基于残缺的摘要，质量大打折扣。建议在上下文进入中后段时就主动执行，而不是等它自己压缩。别让单个会话拖太长，也不要指望 `/compact` 保住记忆。
